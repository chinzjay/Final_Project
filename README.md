...
- **UNWTO**
    - The dataset from UNWTO compiles figures from national and international institutions including national tourism administrations, national statistical offices and central          banks and border statistics. Here is the link to the [UNWTO dataset](https://www.unwto.org/statistic/basic-tourism-statistics).
- **World Economic Indicators dataset from Kaggle database**
    - This dataset includes some of the important indicators such as Life Expectancy, Ease of doing Business, Population, Tourism etc that drives the global economy. It has been       obtained from the United Nations. Here is the link to the [Economic Indicators dataset](https://www.kaggle.com/bhatvikas/world-tourism-economic-data).

## Technologies 
- Data was initially cleaned using **Excel, Macros** and **Python**.
- Initial dataset was imported into tables in **PostgreSQL**.
- The data was then connected to **Jupyter Notebook** and **Machine Learning** models were implemented.
...
During the initial exploration phase, the data was cleaned using Python and MS Excel to be used for the Machine Learning Models. The cleaned data was combined on the fields *Country* and *Year* and filtered for the year 2000 to make an initial analysis. The data was analyzed using Tableau worksheets and Seaborn. Logistic Regression was then performed on the dataset to identify the Type of Tourism (Personal vs Business) that was popular in the country.

## Database, Data Cleaning and PreProcessing
Database was created in PostgreSQL and stored in Amazon Web Services (AWS) to store the static data for analyis. The database has two tables: *inbound_tourism* and *world_indicators*. Both the tables were merged using joins to the *merged_data* table on the columns *Country* and *Year*. The tables were configured using [this](database/schema.sql) schema. 
...
It was used to plot pairwise relationships in the dataset. By default, this function creates a grid of Axes so that each numeric variable in data is shared across the y-axes across a single row and the x-axes across a single column. Here is the visualization plotted using our dataset. 
...
- The training set consisted of 951 train samples which comprised 75% of the total data. 
- The testing set consisted 318 test sampleswhich comprised 25% of the total data.
